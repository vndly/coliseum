...
- `components/` — the shared presentational pieces. `die_face.vue` is the only one: a die face drawn with the same pip layout the real dice carry, used wherever the interface has to count something. Given a `skin` it is drawn in one of the dice's own colours instead of bone, which is what the lobby's colour picker offers.
- `audio/` — browser audio lifecycle for a match. `match_audio.ts` owns the looping music, overlapping roll voices, reusable verdict and result players, autoplay recovery and teardown; the match screen decides which game event each player represents.
- `match/` — Firestore, as plain classes and functions. `match_client.ts` is one player's connection to one match; `match_state.ts` holds the stored shapes and reads them back out; `rules.ts` is the game itself, as pure functions over that state; `firebase.ts` initialises the app and signs in; `codes.ts` draws and normalises join codes; `bots.ts` seats the players nobody is behind and decides their turns; `open_matches.ts` follows the matches the lobby offers a seat in.
- `scene/` — the Three.js scene, as plain classes. `dish_scene.ts` owns the renderer, camera, lighting and render loop; `dish.ts` and `table.ts` build the two objects; `dimensions.ts` holds every measurement and colour; `die_skins.ts` holds the palette a die can be painted in, each swatch carrying its own pip colour; `die_state.ts` holds the shapes a die crosses the network in, and the pip layout that both draws a die and reads the value off one.
